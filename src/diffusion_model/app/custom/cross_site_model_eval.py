...
class CrossSiteModelEval(Controller):
    ...
    def control_flow(self, abort_signal: Signal, fl_ctx: FLContext):
        try:
            ...
            self.log_info(fl_ctx, f"Beginning model validation with clients for {self._participating_clients}.")
    ...
    def _locate_server_models(self, fl_ctx: FLContext) -> bool:
        ...
        for name in server_model_names:
            ...
            try:
                ...
                self.log_exception(
                    fl_ctx,
                    f"Unable to save shareable contents of server model {unique_name}",
                )
                self.system_panic(
                    f"Unable to save shareable contents of server model {unique_name}",
                    fl_ctx,
    ...
    def _send_validation_task(self, model_name: str, fl_ctx: FLContext):
        ...
        # We need to see which model needs to be validated.
        shareable = Shareable()

        self.log_info(
            fl_ctx,
            f"Sending {model_name} model to all participating clients for validation on {self._validation_task_name}.",
        )

        # Create validation task and broadcast to all participating clients.
        targets = []
        for client in self._participating_clients:
            if client not in self._processed_clients:
                targets.append(client)
                self._processed_clients.add(client)
    ...
    def _accept_val_result(
        ...
        if rc and rc != ReturnCode.OK:
            ...
            try:
                ...
                self.log_info(
                    fl_ctx,
                    f"Client {client_name} sent results for validating {model_owner} model.",
                )
            except ValueError as v_e:
                reason = (
