...
class FLIP_Parent(ABC):
    ...
    @abstractmethod
    def send_metrics_value(self, label: str, value: float, fl_ctx: FLContext) -> None:
...
class _FLIPDev(FLIP_Parent):
    ...
    def send_metrics_value(self, label: str, value: float, fl_ctx: FLContext) -> None:
        self.logger.info(
            "[DEV] send_metrics_value is not supported in LOCAL_DEV mode."
            f"Details of the function call: sending metric {label} = {value}."
        )

    @override
    def handle_metrics_event(self, event_data: Shareable, global_round: int, model_id: str) -> None:
        self.logger.info(
            "[DEV] handle_metrics_event is not supported in LOCAL_DEV mode."
            f"Details of the function call: handling metrics event for global round {global_round}"
        )
