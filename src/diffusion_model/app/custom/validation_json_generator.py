            ...
            if val_results:
                try:
                    ...
                    if dxo.data_kind == DataKind.METRICS:
                        ...
                        stage = list(dxo.data.keys())[0]
                        metrics = dxo.data[stage]
                        self._val_results[data_client][model_owner] = {stage: metrics}
                        ...
                        if not os.path.exists(cross_val_res_dir):
                            self.log_info(fl_ctx, f"Creating directory {cross_val_res_dir})...")
                            os.makedirs(cross_val_res_dir)
                        res_file_path = os.path.join(cross_val_res_dir, self._json_file_name)
                        self.log_info(fl_ctx, f"Writing validation results to {res_file_path}...")
                        # If we have results there, we load them to not loose them.
                        pre_existing_results = {}
                        if os.path.exists(res_file_path):
                            with open(res_file_path, "r") as f:
                                pre_existing_results = json.load(f)
                            self.log_info(fl_ctx, f"Loaded preexisting results: {pre_existing_results}")
                            pre_existing_results = self.update_results(pre_existing_results, self._val_results)

                        # Write the merged results back to file
                        with open(res_file_path, "w") as f:
                            json.dump(pre_existing_results, f, indent=4)
                    else:
                        self.log_error(
