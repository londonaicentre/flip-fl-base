...
class CleanupImages(Executor):
    def __init__(self):
        ...
        All the images used for the evaluation are deleted to prevent the build-up of unnecessary
        files on the storage space. Executing at the start of a run ensures that any running code
    ...
    def execute(self, task_name: str, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal):
        try:
            if task_name == FlipTasks.POST_TASK:
                cwd = os.getcwd()
            ...
            if task_name == FlipTasks.INIT_TASK or task_name == FlipTasks.POST_TASK:
                if not FlipConstants.LOCAL_DEV:
