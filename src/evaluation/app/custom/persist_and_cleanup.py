...
class PersistToS3AndCleanup(FLComponent):
    def __init__(self, model_id: str, persistor_id: str = AppConstants.DEFAULT_PERSISTOR_ID, flip: FLIP = FLIP()):
        """The component that is executed post task.

        The PersistToS3AndCleanup workflow saves the aggregated model (once run has finished) to an S3 bucket, and
        then deletes files created as part of the run

        Args:
            model_id (str): ID of the model that the run is being performed under.
    ...
    def execute(self, fl_ctx: FLContext):
        try:
            ...
            # This shouldn't really be here in this case.
